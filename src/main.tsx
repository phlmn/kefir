...
import {calculateTaps, frequencyResponse, getPyodide, minimumPhase} from "./fir";
...
function App() {
  ...
  const latency_estimation = taps?.map((x, i) => x*x / (taps_total || 0) * i * (1000 / 48000)).reduce((x, acc) => x + acc);

  const dbToAxis = (x: Array<{ x: number; y: number }>) => x.map(x => ({...x, y: x.y / 20}));
  const degToAxis = (x: Array<{ x: number; y: number }>) => x.map(x => ({...x, y: x.y / 180}));
      ...
      <div>
        ...
        >
          ...
          >
            ...
            <VictoryScatter
              data={dbToAxis(filterDefs.map((def) => ({
                x: def.frequency,
                y: def.gain,
              })))}
